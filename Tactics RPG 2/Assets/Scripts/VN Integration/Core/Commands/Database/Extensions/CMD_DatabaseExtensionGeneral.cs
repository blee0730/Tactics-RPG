...
public class CMD_DatabaseExtensionGeneral : CMD_DatabaseExtension
{
    ...
    private static void LoadNewDialogueFile(string[] data)
    {
        ...
        if(file == null)
        {
            Debug.LogWarning($"File '{filePath}' could not be loaded from dialogue files. Please ensure it exists within the '{FilePaths.resources_dialogueFiles}' resources foler.");
            return;
        }
