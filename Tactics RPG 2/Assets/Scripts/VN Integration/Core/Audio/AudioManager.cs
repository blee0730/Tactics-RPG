...
public class AudioManager : MonoBehaviour
{
    ...
    public const float MUTED_VOLUME_LEVEL = -80;
    ...
    public AudioSource PlaySoundEffect(string filePath, AudioMixerGroup mixer = null, float volume = 1, float pitch = 1, bool loop = false)
    {
        ...
        if(clip == null)
        {
            Debug.LogError($"Could not load audio file '{filePath}'. Please make sure this exists in Resources!");
            return null;
        }
    ...
    {
        volume = muted ? AudioManager.MUTED_VOLUME_LEVEL : audioFalloffCurve.Evaluate(volume);
