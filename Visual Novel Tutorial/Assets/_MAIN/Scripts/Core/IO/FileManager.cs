...
public class FileManager
{
    ...
    public static void Save(string filePath, string JSONData, bool encrypt = false)
    {
        ...
        {
            Debug.LogError($"FAILED TO SAVE FILE '{filePath}' Please see the ocnsole for error details.");
            return;
        }
        if(encrypt)
        {
            ...
            byte[] encreyptedBytes = XOR(dataBytes, keyBytes);

            File.WriteAllBytes(filePath, encreyptedBytes);
        }
        else
        {
